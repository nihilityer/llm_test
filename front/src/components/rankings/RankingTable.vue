    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr
          ...
          :key="entry.website_id"
          class="table__row table__row--clickable"
          @click="emit('select', entry.website_id)"
        >
          ...
          <td class="fw-600">{{ entry.website_name }}</td>
          <td class="text-secondary text-sm">{{ entry.domains.join(', ') }}</td>
